...
/imagery/api/v1.0/avalanche_risks/<string:longitude_initial>/<string:latitude_initial>/<string:longitude_final>/<string:latitude_final>
	...
	\subsection{The API Server Handling the Request}
		...
		When an array of data points is returned from the raster interface, a half-transparent 50\% PNG image will be constructed with the Pillow library \footnote{https://python-pillow.org/} from the two-dimensional array. Each row of the array provides the colouring for a column of pixels of the image, and vice versa. The colouring of pixels is based on the returned risk level, and as follows:
		\begin{figure}[h]
		\centering
		\begin{tabular}{ | c | c | }
  		\hline
