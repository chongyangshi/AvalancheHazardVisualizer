...
\begin{description}
\item[Terrain Picker] - A dropdown menu for switching between different terrain shape sources provided by the self-hosted terrain server. Due to difficulties in building terrain shape files from very large rasters, the terrain for each observation area (Glencoe, Lochaber, etc) had to be built separately, prompting this feature. This dropdown can be omitted by switching to the terrain server provided by the Cesium's maintainers.
