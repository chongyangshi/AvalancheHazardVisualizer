...
\begin{figure}[h]
		...
		\caption{\label{fig:stage1bennevis} The avalanche hazard visualisation of the same area mapped in Figure \ref{fig:mapping}, generated in the 3D terrain viewer using the same avalanche forecast by both the essential framework (dynamic risk only, left) and the final product (both static terrain and dynamic risk with new colour coding, right).}
\end{figure}
...
In order to effectively track and manage the workload, the course of the project was divided into a requirement gathering stage and four development stages: the essential framework, static risk integration, risk-adjusted pathfinding, and evaluation \& testing. As the product from each stage is able to function independently of work done in later stages, unforeseen difficulties encountered in the research and development process can be effectively mitigated. The stages are described in Chapter \ref{ch:problem}.
...
This chapter provides the details of the theories in avalanche forecasting, spatial analysis, risk representation and search algorithms applied across different stages of the project. Section \ref{sec:dynamic-factors} will be utilised in the construction of essential framework in \textbf{Stage 1}, which is built-on in \textbf{Stage 2} with Sections \ref{sec:static-factors} and \ref{sec:chosen-colour-coding}. Section \ref{sec:risky-pathfinding} provides the basis of algorithm design and implementation in \textbf{Stage 3}. Finally, Section \ref{sec:stats-eval} provides theoretical basis of statistically evaluating the risk model devised against past efforts in \textbf{Stage 4}.
...
As shown by (\ref{eq:curvature-curve}), the numerical model of the spline is based on the curve of function $y = C^3 + 0.5$, but with a range of $[0, 1]$ to form a numerically-valid risk factor. Extreme curvatures represented by $C > 0.7937$ or $C < -0.7937$ (curvature radius $r < 1.26m$) will be capped at extreme risk probabilities $P = 1$ and $P = 0$ respectively. We believe that the shape of the spline models the aforementioned effects of changing terrain curvature well: as the terrain levels off from a concave, the avalanche risk increases steeply until approaching the linear region, where there is no significance change in the risk level; and as the terrain curvature moves away from this region into convex, the avalanche risk increases again steeply.

In practise, the curvature value $C$ can be easily computed by taking the second derivative of (\ref{eq:surface-polynomial}) with respect to the aspect angle (calculated by $\theta = atan(\frac{H}{G})$) in the fitted neighbourhood \cite[Ch. 6.2.2]{de2007geospatial}, which is:
...
\begin{figure}[h]
		...
		\caption{\label{fig:new-system} The architecture of the system. All components in Data Interface, API Servers and Front-end are implemented from scratch during this project, except the Terrain Server.}
\end{figure}
...
Serving as the intermediate between the user and the data processors, the API Servers level provide two categories of APIs required for implementing the actions of the application: Data API and Imagery API, both of which will be covered in this section. 
...
\end{lstlisting}
{\small Required: all parameters except {\ttfamily forecast\_date}, defaulting today's date.}
...
In usability testing, another experienced mountaineer was invited to use both the traditional map-and-forecast method and the new application to plan routes in the Lochaber area, as if climbing on a day with recorded avalanches. Without knowledge of the recorded avalanches, the user successfully avoided planning a route in the avalanched area with both methods. The user's comments and any associated changes we made are shown in Figure \ref{fig:user-testing-table}.
...
More dynamic data layers will be useful, such as snowfall levels and weather. & Data will have to be specially sourced and updated. For future work due to insufficient time. \\ \hline
Risk of cornice avalanche near ridges not sufficiently considered. & Can be taken into account in the static risk model,  difficulties in determining ridges require future work. \\ \hline
Use of crowdsource avalanche records, and risk area labelings by experts could help enhance the risk model. & This is possible if this application can be commercialised. \\ \hline
The data model does not consider vertical effects of risk, e.g. a high release risk area would increase the risk of areas below it. & With a decay-by-distance function, this should be easily computable, but requires adjustment of the decay parameters require repeated computations, which is beyond our current computational capabilities. \\ \hline
...
\multicolumn{2}{|p{\linewidth}|}{The Pathfinder may not be an appropriate feature, as any inappropriate routes returned by the pathfinder would affect the user's confidence in the entire application. Users should probably be encouraged to work out the route based on the risk map of the application and their own experience. However, this functionality can be a very useful training tool.} \\ \hline
\end{tabular}
\caption{\label{user-testing-table}Comments and suggestions made by the potential user during usability testing, and any associated action we took to improve the application.} 
\end{figure}
...
Before comparing the accuracy of our model against the performance of release area estimation of other models, it is important to note that percentages values cannot be compared directly due to the differences in size of study areas. Similar to the previous study in the Scottish Highlands by Purves \textit{et al.} \cite{Purves2003343}, our study area is significantly larger than many others, and the lack of precisely recorded avalanches added another layer of complications. Within an extent of 25 meters, covering 0.5\% of study area, our model produces a comparable performance (70.12\%) to others discussed in Section \ref{sec:modelling-avalanche}, as shown in Figure \ref{fig:eval-compare}.
