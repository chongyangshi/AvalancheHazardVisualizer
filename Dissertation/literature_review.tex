...
Many of the efforts above have been able to produced hazard maps that could visualise locations with significant avalanche hazards to a map-reading user, as shown in Figure (...an image of different visualisations...). However, most of these maps are in 2D only, and are static once computed, providing no interactive capabilities. To our knowledge, only Pistocchi and Notarnicola \cite{Pistocchi2013} constructed a static 3D model with an avalanche risk overlay, similar to the 3D model constructed in this project. The interactive 3D model proposed by this project would also provide capabilities of displaying additional image layers and map features (such as a planned route) "on the fly", and the ability to observe from different angles and distances.
